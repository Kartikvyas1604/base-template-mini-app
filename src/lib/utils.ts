...
import { APP_BUTTON_TEXT, APP_DESCRIPTION, APP_ICON_URL, APP_NAME, APP_OG_IMAGE_URL, APP_PRIMARY_CATEGORY, APP_SPLASH_BACKGROUND_COLOR, APP_TAGS, APP_URL, APP_SPLASH_URL } from './constants';
...
export async function getFarcasterMetadata(): Promise<FrameManifest> {
  ...
  return {
    ...
    frame: {
      version: "1",
      name: APP_NAME ?? "TapMint",
      iconUrl: APP_ICON_URL,
      homeUrl: APP_URL,
      imageUrl: APP_OG_IMAGE_URL,
      buttonTitle: APP_BUTTON_TEXT ?? "Start Connection",
      splashImageUrl: APP_SPLASH_URL,
      splashBackgroundColor: APP_SPLASH_BACKGROUND_COLOR,
      description: APP_DESCRIPTION,
      primaryCategory: APP_PRIMARY_CATEGORY,
